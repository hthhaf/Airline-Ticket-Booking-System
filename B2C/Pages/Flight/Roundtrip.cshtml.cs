...
{
    public class RoundtripModel : PageModel
    {
        ...
        public List<FlightModel> flightsReturn { get; set; }
        
        public List<FlightWithPrice> flightsWithPriceDep { get; set; }
        public List<FlightWithPrice> flightsWithPriceReturn { get; set; }
        ...
        public List<FlightRoundtrip> ListFlightRoundtrips { get; set; }
        public UserInfo UserLoginInfo { get; set; }
        public List<Passenger> Passengers { get; set; }
        public Booking Abooking { get; set; }
        public RoundtripModel(ApplicationDbContext dbContext, UserManager<IdentityUser> userManager, IMailService _MailService, IViewRenderService viewRenderService)
        {
        ...
        public async Task OnGet(string type, string departure, string arrival, string departureDate, string returnDate, int passenger, int adult, int children)
        {
            try
            {
                ListFlightRoundtrips = new List<FlightRoundtrip>();
                ...
                flights = data;
                flightsWithPriceDep = new List<FlightWithPrice>();
                ...
                foreach (var flight in flights)
                {
                    ...
                    foreach (var ticketPrice in ticketPrices)
                    {
                        ...
                        flightsWithPriceDep.Add(flightWithPrice);
                    }

                }

                var datareturn = _dbContext.Flights
                    .Where(x => 
                        x.Arrival == departure &&
                        x.Departure == arrival &&
                        x.DepartureTime.Date.ToString() == returnDate &&
                        x.AvailableSeats >= passenger)
                    .ToList();

                flightsReturn = datareturn;

                flightsWithPriceReturn = new List<FlightWithPrice>();

                foreach (var flight in flightsReturn)
                {
                    var ticketPrices = _dbContext.TicketsPrices.Where(tp => tp.FlightCode == flight.Code).ToList();
                    ...
                    foreach (var ticketPrice in ticketPrices)
                    {
                        ...
                        flightsWithPriceReturn.Add(flightWithPrice);
                    }

                }

                
                if (flightsWithPriceReturn.Count > 0)
                {
                    foreach (var flighdep in flightsWithPriceDep)
                    {
                        foreach (var flightarr in flightsWithPriceReturn)
                        {
                            var flightRo = new FlightRoundtrip
                            {
                                FlightDep = flighdep,
                                FlightReturn = flightarr,
                                amount = flighdep.amount + flightarr.amount,
                                tax= flighdep.tax + flightarr.tax,
                            };

                            ListFlightRoundtrips.Add(flightRo);
                        }
                    }
                }
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            try
            {
                ...
                if (isSave)
                {
                    ...
                    string body = await _viewRenderService.RenderToStringAsync("_Roundtrip", new Template { Booking = Booked, Pax = Pax });
        ...
        private async Task<IActionResult> UpdateAvailableSeats(string flightId)
        {
            ...
            if (flight != null)
            {
                int availableSeats = flight.AvailableSeats - paxCount;
                ...
                if (await TryUpdateModelAsync(booking, "Abooking"))
                 {
                    ...
                    booking.ReservationType = "Roundtrip";
